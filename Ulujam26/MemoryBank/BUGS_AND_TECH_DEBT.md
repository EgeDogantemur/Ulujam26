...
| Ana gameplay scene'i yok | Critical | `SampleScene` geçici başlangıç noktası | `Assets/CHROMAVOID/Scenes/` altında dedicated arena scene oluştur | Optional |
...
| --- | --- | --- | --- | --- |
| Shader denemeleri `_AshAndPause/shadermat` altında isimlendirme standardı dışında | Medium | Mevcut assetleri referans olarak bırak | Production shader/materialleri `Assets/CHROMAVOID/Shaders` ve `Materials` altına taşı | Optional |
| Starter Assets ve oyun kodu ayrımı henüz yapılmadı | Medium | Starter Assets'e dokunmadan yeni scriptleri ayrı klasöre yaz | `Assets/CHROMAVOID/` üretim klasörü standardını kullan | Optional |
